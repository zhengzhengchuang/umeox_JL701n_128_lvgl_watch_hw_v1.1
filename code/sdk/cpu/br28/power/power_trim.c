...
#define AT_RAM      AT(.pwr_trim_ram_code)
...
static u8 wiovdd_trim(u8 trim, s16 *adc_wiovdd_voltage)
{
    ...
    if (trim) {
        for (; wiovdd_lev > 3; wiovdd_lev--) {
            P33_CON_SET(P3_ANA_CON5, 3, 3, wiovdd_lev);
            delay(700);//延时50us
            adc_wiovdd_voltage[wiovdd_lev] =  adc_get_voltage_preemption(AD_CH_IOVDD);
        }
...
u8 miovdd_trim(u8 trim, s16 *adc_miovdd_voltage)
{
    ...
    if (trim) {
        for (; miovdd_lev > 3; miovdd_lev--) {
            P33_CON_SET(P3_ANA_CON5, 0, 3, miovdd_lev);
            delay(700);//延时50us
            adc_miovdd_voltage[miovdd_lev] = adc_get_voltage_preemption(AD_CH_IOVDD);
        }
...
u32 iovdd_trim(u8 trim, u8 *miovdd_lev, u8 *wiovdd_lev)
{
    ...
    u32 check_vbat =  adc_get_voltage_preemption(AD_CH_VBAT) * 4;
    printf("check_vbat = %d\n", check_vbat);
    if (check_vbat < 3700) {
        printf("%s    %d\n", __func__, __LINE__);
    ...
    if (trim) {
        ...
        if (v == 0) {
            ...
            do {
                WVDD_VOL_SEL(wvdd_lev);
                delay(2000);//1ms * n
                v = adc_get_voltage_preemption(AD_CH_WVDD);
                if (v > WVDD_VOL_TRIM) {
                    break;
                }
                wvdd_lev ++;
...
static u8 pvdd_trim(u8 trim)
{
    ...
    if (trim) {
        ...
        delay(2000);//1ms
        v = adc_get_voltage_preemption(AD_CH_PVDD);
