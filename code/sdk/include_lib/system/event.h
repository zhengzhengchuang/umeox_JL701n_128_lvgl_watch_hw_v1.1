...
#define DEVICE_EVENT_CHARGE_CALIBRATION        (('C' << 24) | ('A' << 16) | ('L' << 8) | '\0')
#define DEVICE_EVENT_CONFIG_TOOLS      (('C' << 24) | ('F' << 16) | ('T' << 8) | '\0')
...
struct charge_calibration_event {
    u8 event;
    u32 value;
};

struct config_tools_event {
    u8 event;
    u32 value;
};
...
struct sys_event {
    ...
    union {
        ...
        struct charge_calibration_event charge_calibration;
        struct config_tools_event config_tools;
