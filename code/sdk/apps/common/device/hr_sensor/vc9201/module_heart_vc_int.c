...
const uint32_t mcuOscData = 36000;
...
static const uint8_t cash_num[20] = 
{
	1, 3, 6, 8, 10, 12, 15, 17, 19, 21, \
	24, 26, 28, 30, 33, 35, 37, 39, 42, 43,
};
...
/* PD switch */
extern bool BuiltPD_or_ExternalPD;
uint8_t BuiltPD_preValue = 0;
uint8_t PD_Change = 0;

static const int hr_interrupt_io = \
	IO_PORTB_02;
 ...
 *   
 **/
void vcHr02_process(AlgoSportMode sportMode)
{
	...
	if(vcHr02IRQFlag)
	{
			...
			if(vcHr02.vcFifoReadFlag || vcHr02.vcPsFlag)
			{
				...
				*/
				...
				for(uint8_t i=0;i<20;i++)
				{
					...
					//TODO:装机三轴的调整
					xData[i] = xData[cash_num[i]] >> 4; 
					yData[i] = yData[cash_num[i]] >> 4;
					zData[i] = zData[cash_num[i]] >> 4;
				}
#endif			
				...
				{	
					...
					if(vcHr02.wearStatus == VCHR02WEARST_ISWEAR)
					{
						...
						for(algoCallNum= 0; algoCallNum < ppgLength; algoCallNum++)
						{
							...
							Algo_Input(&algoInputData, 1000/vcHr02SampleRate, sportMode, 1, 0 );
						}
				...
				if(vcHr02.vcFifoReadFlag)
				{
					...
					{					
						...
						{					
							...
							vcSpo2Value = spo2Algo(vcRedPPG, vcIrPPG, 1);
#if 0//GsensorEn						
							vcSportFlag = vcSportMotionCalculate(xData[algoCallNum], yData[algoCallNum],zData[algoCallNum]);
#endif						
							if((!vcSportFlag) && (vcSpo2Value > 0.0f))
							{
								real_spo = (int)vcSpo2Value;
		...
		else if(vcHr02.workMode == VCWORK_MODE_CROSSTALKTEST)
		{
			...
			if (PD_Change == 5)
			{
				BuiltPD_preValue = vcHr02.sampleData.preValue[0];
				BuiltPD_or_ExternalPD = false;
				vcHr02Init(&vcHr02,VCWORK_MODE_CROSSTALKTEST);
				PD_Change = 10;
			}
			vcHr02.sampleData.preValue[0] = vcHr02.sampleData.preValue[0] - BuiltPD_preValue;
			if (vcHr02.sampleData.preValue[0] < 0)
			{
				vcHr02.sampleData.preValue[0] = 0;
			}
			/* If Pass: */
...
/*------------------INSERT YOUR CODE HERE-----------------*/
	/*配置中断*/ 
	gpio_set_pull_up(hr_interrupt_io, 0);
	gpio_set_pull_down(hr_interrupt_io, 1);
	gpio_set_direction(hr_interrupt_io, 1);
	gpio_set_die(hr_interrupt_io, 1);
 ...
 **/
/****************************timer3*****************************/
extern u32 get_user_time_cnt(void);
uint32_t vcHr02GetRtcCountFromMCU(void)
{
/*------------------INSERT YOUR CODE HERE-----------------*/
	return get_user_time_cnt();
}

void vcHr02DevInit(void)
{
	u8 id_tmp = 0x00;
	vcHr02ReadRegisters(0x00, &id_tmp, 1);
    printf("%s:id = 0x%02x\n", __func__, id_tmp);
