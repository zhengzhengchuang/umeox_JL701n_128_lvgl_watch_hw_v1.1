...
static const u32 long_8s_cnt = 800;
...
void set_curr_key_event(u16 keyevent){
    usr_key_value = keyevent;
}

u16 get_curr_key_event(){
    return usr_key_value;
}
...
//===== 按键消抖结束, 开始判断按键类型(单击, 双击, 长按, 多击, HOLD, (长按/HOLD)抬起)
    ...
    { 	//cur_key = last_key -> 没有按键按下/按键长按(HOLD)
            ...
            {  //有按键需要消息需要处理
                ...
                { 
                    ...
                    if (scan_para->click_cnt >= 5) {
                        ...
                        key_event = KEY_EVENT_FOURTH_CLICK;  //四击
        ...
        {  //last_key = valid_key; cur_key = valid_key, press_cnt累加用于判断long和hold
            ...
            // else if(scan_para->press_cnt == long_8s_cnt)
            // {
            //     key_event = KEY_EVENT_LONG_8S;
            // }
...
_scan_end:  
    ...
    extern void SetPowerOnStatus(bool status);
    if(cur_key_value == NO_KEY)
        SetPowerOnStatus(false);
...
{
    if (port_active_is_keyio_query(port)) {
        is_key_active = 35;      //35*10Ms
    }
}
#endif
