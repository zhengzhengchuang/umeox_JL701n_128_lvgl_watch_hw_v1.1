...
static void menu_display_cb(lv_obj_t *obj)
{
    ...
    widget_img_para.file_img_dat = alarm_01_index;
    widget_img_para.img_click_attr = false;
    widget_img_para.event_cb = NULL;
    lv_obj_t *alarm_remind_icon = common_widget_img_create(&widget_img_para, NULL);
    lv_obj_align(alarm_remind_icon, LV_ALIGN_TOP_MID, 0, 60);

    int time_format = GetVmParaCacheByLabel(vm_label_time_format);

    uint8_t am_or_pm;
    uint8_t alarm_id = GetAlarmIsOnId();
    uint8_t alarm_time_hour = Alarm_Info.alarm_union[alarm_id].bit_field.alarm_hour;
    uint8_t alarm_time_minute = Alarm_Info.alarm_union[alarm_id].bit_field.alarm_minute;
    ...
    num_str_para.num_obj_x = 104;
    num_str_para.num_obj_y = 184;
    num_str_para.p_num_str = alarm_time_str;
    ...
    int16_t end_y = common_widget_num_str_create(&num_str_para);
  
    widget_img_para.img_parent = obj;
    if(menu_align == menu_align_left)
        widget_img_para.img_x = end_y + 10;
    else
        widget_img_para.img_x = 104 - 36 - 10;
    widget_img_para.img_y = 210;
    widget_img_para.file_img_dat = comm_icon_24_index + am_or_pm;
    widget_img_para.img_click_attr = false;
    widget_img_para.event_cb = NULL;
    ...
    widget_img_para.event_cb = alarm_remind_exit_cb;
