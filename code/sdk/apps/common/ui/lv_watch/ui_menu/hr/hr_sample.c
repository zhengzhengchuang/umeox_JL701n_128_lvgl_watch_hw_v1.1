...
static void hr_sample_container_create(lv_obj_t *obj)
{
    ...
    hr_sample_container = common_widget_obj_create(&widget_obj_para);
    lv_obj_add_event_cb(hr_sample_container, scroll_cb, LV_EVENT_SCROLL, NULL);
...
static void hr_range_chart_create(lv_obj_t *obj)
{
    ...
    widget_chart_para.chart_priy_range_min = hr_chart_min;
    widget_chart_para.chart_priy_range_max = hr_chart_max;
    ...
    widget_chart_para.chart_ext_y_array = hr_sample_chart_points[0];
    widget_chart_para.chart_series_color = hr_sample_chart_color;
    widget_chart_para.chart_series = hr_sample_chart_series;
    widget_chart_para.chart_main_opax = LV_OPA_0;
    widget_chart_para.chart_main_color = lv_color_hex(0x000000);
    lv_obj_t *hr_sample_chart = common_widget_chart_create(&widget_chart_para);
    lv_obj_align(hr_sample_chart, LV_ALIGN_TOP_MID, 0, (3*LCD_HEIGHT - chart_h)/2);
    lv_obj_clear_flag(hr_sample_chart, LV_OBJ_FLAG_SCROLLABLE);

    const uint8_t hr_sample_bar_w = 8;
    const uint8_t hr_sample_bar_inv = 4;
    
    widget_obj_para.obj_parent = hr_sample_chart;
    widget_obj_para.obj_width = hr_sample_bar_w;
    widget_obj_para.obj_bg_opax = LV_OPA_100;
    widget_obj_para.obj_bg_color = lv_color_hex(0xfe375e);
    widget_obj_para.obj_border_opax = LV_OPA_0;
    widget_obj_para.obj_border_width = 0;
    widget_obj_para.obj_border_color = lv_color_hex(0x000000);
    widget_obj_para.obj_radius = hr_sample_bar_w/2;
    ...
    u8 HIdx = (w_hr.CurIdx)/(60/Hr_Inv_Dur);
    
    for(uint8_t i = 0; i < (HIdx + 1); i++)
    {
        hr_sample_min = HrPara.hr_min[i];
        hr_sample_max = HrPara.hr_max[i];

        if(hr_sample_max < hr_sample_min)
            continue;

        if(hr_sample_min == 0 && hr_sample_max == 0)
            continue;
        ...
        hr_sample_bar_h = (hr_sample_max - hr_sample_min)*hr_pixel_per;
        if(hr_sample_bar_h < hr_pixel_per)
            hr_sample_bar_h = hr_pixel_per*4;
        widget_obj_para.obj_height = hr_sample_bar_h;

        lv_obj_t *hr_sample_bar = common_widget_obj_create(&widget_obj_para);
        lv_obj_align(hr_sample_bar, LV_ALIGN_TOP_LEFT, (hr_sample_bar_w + hr_sample_bar_inv)*i, \
...
static void menu_create_cb(lv_obj_t *obj)
{
    ...
    {
        ui_act_id_t left_act_id = ui_act_id_activity;
...
static void menu_display_cb(lv_obj_t *obj)
{
    ...
    widget_img_para.img_parent = hr_sample_container;
    widget_img_para.file_img_dat = hr_00_index;
    widget_img_para.img_click_attr = false;
    widget_img_para.event_cb = NULL;
    lv_obj_t *hr_sample_icon = common_widget_img_create(&widget_img_para, NULL);
    ...
    widget_data_para.data_parent = hr_sample_container;
    widget_data_para.num_addr_index = comm_num_24x40_wh_00_index;
    widget_data_para.data_align = widget_data_align_center;
    widget_data_para.user0_para_valid = true;
    widget_data_para.user0_num_addr_index = comm_num_24x40_re_00_index;
    common_data_widget_create(&widget_data_para, widget_data_type_hr, &hr_sample_val);

    widget_img_para.img_parent = hr_sample_container;
    widget_img_para.img_x = 216;
    widget_img_para.img_y = 117;
    widget_img_para.file_img_dat = hr_01_index;
    widget_img_para.img_click_attr = false;
    widget_img_para.event_cb = NULL;
    common_widget_img_create(&widget_img_para, NULL);

    hr_GIF_cnt = 0;
    widget_img_para.file_img_dat = hr_GIF_00_index;
    hr_GIF_icon = common_widget_img_create(&widget_img_para, &hr_GIF_dsc_idx);
    lv_obj_align(hr_GIF_icon, LV_ALIGN_TOP_MID, 0, 224);

    widget_img_para.file_img_dat = hr_02_index;
    lv_obj_t *hr_02_icon = common_widget_img_create(&widget_img_para, NULL);
