...
static void tips_label_hid_timeout_cb(lv_timer_t *timer)
{
    ...
    {
        lv_timer_del(tips_label_hid_timer);
...
static void tasbih_etime_confirm_cb(lv_event_t *e)
{
    ...
    uint8_t stime_hour = GetTasbih_stime_hour();
    uint8_t stime_minute = GetTasbih_stime_minute();
    uint8_t etime_hour = GetTasbih_etime_hour();
    uint8_t etime_minute = GetTasbih_etime_minute();

    uint32_t TasbihR_STimestamp = stime_hour*3600 + stime_minute*60;
    uint32_t TasbihR_ETimestamp = etime_hour*3600 + etime_minute*60;
 
    uint32_t ST = TasbihR_STimestamp;
    uint32_t ET = TasbihR_ETimestamp;

    u32 TasbihR_TotalInvTime;
    if(ET >= ST)
        TasbihR_TotalInvTime = ET - ST;
    else
        TasbihR_TotalInvTime = ET + SEC_PER_DAY - ST; 

    uint32_t TasbihR_SetInvTime = \
        TasbihR_info.TasbihR_SetInvTime;

    if(TasbihR_SetInvTime > TasbihR_TotalInvTime)
    {
        lv_obj_clear_flag(tips_label_container, LV_OBJ_FLAG_HIDDEN);

        if(!tips_label_hid_timer)
            tips_label_hid_timer = lv_timer_create(tips_label_hid_timeout_cb, 3*1000, NULL);

        return;
    }

    TasbihR_info.TasbihR_STimestamp = TasbihR_STimestamp;
    TasbihR_info.TasbihR_ETimestamp = TasbihR_ETimestamp;
    TasbihR_info.TasbihR_TotalInvTime = TasbihR_TotalInvTime;

    TasbihRInfoParaUpdate();

    ui_menu_jump(ui_act_id_tasbih_main);
...
static void menu_display_cb(lv_obj_t *obj)
{
    ...
    widget_label_para.label_text = get_lang_txt_with_id(lang_txtid_end_time);
    lv_obj_t *tasbih_etime_label = common_widget_label_create(&widget_label_para);
    lv_obj_align(tasbih_etime_label, LV_ALIGN_TOP_MID, 0, 40);

    widget_img_para.img_parent = obj;
    widget_img_para.file_img_dat = comm_icon_37_index;
    widget_img_para.img_click_attr = false;
    widget_img_para.event_cb = NULL;
    tasbih_etime_hour_sel = \
        common_widget_img_create(&widget_img_para, &tasbih_etime_hour_dsc);
    lv_obj_align(tasbih_etime_hour_sel, LV_ALIGN_LEFT_MID, 32, 11);

    widget_img_para.file_img_dat = comm_num_30x50_wh_10_index;
    lv_obj_t *tasbih_etime_colon = common_widget_img_create(&widget_img_para, NULL);
    lv_obj_align(tasbih_etime_colon, LV_ALIGN_LEFT_MID, 176, 10);

    widget_img_para.file_img_dat = comm_icon_38_index;
    tasbih_etime_minute_sel = \
        common_widget_img_create(&widget_img_para, &tasbih_etime_minute_dsc);
    lv_obj_align(tasbih_etime_minute_sel, LV_ALIGN_LEFT_MID, 216, 11);

    widget_roller_para.roller_parent = obj;
    widget_roller_para.roller_width = 120;
    widget_roller_para.roller_height = 0;
    widget_roller_para.roller_row_count = 3;
    widget_roller_para.roller_options = time_hour_str;
    widget_roller_para.roller_mode = LV_ROLLER_MODE_NORMAL;
    widget_roller_para.roller_sel = tasbih_etime_hour;
    widget_roller_para.roller_line_inv = 15;
    widget_roller_para.roller_main_bg_opax = LV_OPA_0;
    widget_roller_para.roller_main_bg_color = lv_color_hex(0x000000);
    widget_roller_para.roller_selected_bg_opax = LV_OPA_0;
    widget_roller_para.roller_selected_bg_color = lv_color_hex(0x000000);
    widget_roller_para.roller_border_width = 0;
    widget_roller_para.roller_border_opax = LV_OPA_0;
    widget_roller_para.roller_border_color = lv_color_hex(0x000000);
    widget_roller_para.roller_main_text_font = &font_common_num_64;
    widget_roller_para.roller_main_text_color = lv_color_hex(0x999999);
    widget_roller_para.roller_selected_text_font = &font_common_num_64;
    widget_roller_para.roller_selected_text_color = lv_color_hex(0xF0D990);
    widget_roller_para.event_cb = NULL;
    widget_roller_para.user_data = NULL;
    tasbih_etime_hour_roller = common_widget_roller_create(&widget_roller_para);
    lv_obj_align(tasbih_etime_hour_roller, LV_ALIGN_LEFT_MID, 32, 10);
    lv_obj_add_event_cb(tasbih_etime_hour_roller, tasbih_etime_hour_roller_cb, LV_EVENT_ALL, NULL);

    widget_roller_para.roller_options = time_min_sec_str;
    widget_roller_para.roller_sel = tasbih_etime_minute;
    widget_roller_para.roller_main_text_color = lv_color_hex(0x000000);
    widget_roller_para.roller_selected_text_color = lv_color_hex(0x000000);
    widget_roller_para.event_cb = NULL;
    widget_roller_para.user_data = NULL;
    tasbih_etime_minute_roller = common_widget_roller_create(&widget_roller_para);
    lv_obj_align(tasbih_etime_minute_roller, LV_ALIGN_LEFT_MID, 216, 10);
    lv_obj_add_event_cb(tasbih_etime_minute_roller, tasbih_etime_minute_roller_cb, LV_EVENT_ALL, NULL);

    widget_img_para.img_parent = obj;
    widget_img_para.file_img_dat = comm_icon_02_index;
    widget_img_para.img_click_attr = true;
    widget_img_para.event_cb = tasbih_etime_confirm_cb;
    ...
    widget_obj_para.obj_bg_opax = LV_OPA_90;
    ...
    widget_label_para.user_text_font = NULL;
    widget_label_para.label_parent = tips_label_container;
    widget_label_para.label_text = get_lang_txt_with_id(lang_txtid_exc_time_range);
    lv_obj_t *tips_label_label = common_widget_label_create(&widget_label_para);
