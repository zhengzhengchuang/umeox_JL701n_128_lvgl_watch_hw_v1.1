...
static void enable_cb(lv_event_t *e)
{
    ...
    ui_menu_jump(ui_act_id_Hcalendar_festivals);
...
static void enable_create(lv_obj_t *obj, uint8_t menu_align)
{
    ...
    widget_label_para.label_text = get_lang_txt_with_id(lang_txtid_festival_reminder);
    ...
    widget_img_para.file_img_dat = comm_icon_00_index + FesRemindersFlag;
    widget_img_para.img_click_attr = true;
    widget_img_para.event_cb = enable_cb;
...
{
    int16_t ctx_w = LCD_WIDTH;
    widget_obj_para.obj_parent = list_ctx_container;
    widget_obj_para.obj_x = 0;
    widget_obj_para.obj_width = ctx_w;
    widget_obj_para.obj_bg_opax = LV_OPA_0;
    widget_obj_para.obj_bg_color = lv_color_hex(0x000000);
    widget_obj_para.obj_border_opax = LV_OPA_0;
    widget_obj_para.obj_border_width = 0;
    ...
    for(uint8_t i = Hijri_New_Year; i < HcalendarFesNum; i++)
    {
        ...
        lv_obj_t *elem_ctx_container = common_widget_obj_create(&widget_obj_para);
        ...
        widget_label_para.user_text_font = NULL;
        widget_label_para.label_parent = elem_ctx_container;
        widget_label_para.label_text = get_lang_txt_with_id(lang_txtid_Hijri_New_Year + i);
        lv_obj_t *Hcale_fes_label = common_widget_label_create(&widget_label_para);
        lv_obj_align(Hcale_fes_label, LV_ALIGN_TOP_MID, 0, 0);
   
        u8 dysnum = Hcale_fes_dysnum[i];
        for(uint8_t j = 0; j < dysnum; j++)
        {
            widget_img_para.img_parent = elem_ctx_container;
            widget_img_para.file_img_dat = hijri_calendar_icon_04_index;
            widget_img_para.img_click_attr = false;
            widget_img_para.event_cb = NULL;
            widget_img_para.img_y = 90;
            widget_img_para.img_x = (ctx_w - dysnum*80)/2 + j*80;
            lv_obj_t *Hcale_day_container = common_widget_img_create(&widget_img_para, NULL);

            memset(HcaleDayStr, 0, sizeof(HcaleDayStr));
            sprintf(HcaleDayStr, "%02d", HcalendarInfo.HFesInfo[i].HFes_day + j);
            num_str_para.parent = Hcale_day_container;
            num_str_para.num_obj_x = 24;
            num_str_para.num_obj_y = 28;
            num_str_para.p_num_str = HcaleDayStr;
            num_str_para.num_str_len = strlen(HcaleDayStr);
            num_str_para.num_obj = NULL;
            num_str_para.num_obj_max = 0;
            num_str_para.num_dsc_idx = NULL;
            num_str_para.file_00_index = comm_num_16x28_ye_00_index;
            common_widget_num_str_create(&num_str_para);
        }
        
        u8 HFes_month = HcalendarInfo.HFesInfo[i].HFes_month;
        widget_label_para.label_w = 300;
        widget_label_para.label_h = Label_Line_Height;
        widget_label_para.long_mode = LV_LABEL_LONG_SCROLL;
        widget_label_para.text_align = LV_TEXT_ALIGN_CENTER;
        ...
        widget_label_para.user_text_font = NULL;
        widget_label_para.label_parent = elem_ctx_container;
        widget_label_para.label_text = get_lang_txt_with_id(lang_txtid_Muharram + HFes_month - 1);
        lv_obj_t *Hcale_fes_month_label = common_widget_label_create(&widget_label_para);
        lv_obj_align(Hcale_fes_month_label, LV_ALIGN_TOP_MID, 0, 165);

        u16 HFes_year = HcalendarInfo.HFesInfo[i].HFes_year;
        memset(HcaleYearStr, 0, sizeof(HcaleYearStr));
        sprintf(HcaleYearStr, "%d", HFes_year);
        num_str_para.parent = elem_ctx_container;
        num_str_para.num_obj_x = 156;
        num_str_para.num_obj_y = 215;
        num_str_para.p_num_str = HcaleYearStr;
        num_str_para.num_str_len = strlen(HcaleYearStr);
        num_str_para.num_obj = NULL;
        num_str_para.num_obj_max = 0;
        num_str_para.num_dsc_idx = NULL;
        ...
        u16 S_GFes_year = HcalendarInfo.GFesInfoS[i].GFes_year;
        u8  S_GFes_month = HcalendarInfo.GFesInfoS[i].GFes_month;
        u8  S_GFes_day = HcalendarInfo.GFesInfoS[i].GFes_day;
        u16 E_GFes_year = HcalendarInfo.GFesInfoE[i].GFes_year;
        u8  E_GFes_month = HcalendarInfo.GFesInfoE[i].GFes_month;
        u8  E_GFes_day = HcalendarInfo.GFesInfoE[i].GFes_day;
        memset(GcaleDateStr, 0, sizeof(GcaleDateStr));
        sprintf(GcaleDateStr, "%02dD  %02dM  %d", S_GFes_day, S_GFes_month, S_GFes_year);
        widget_label_para.label_w = 300;
        ...
        widget_label_para.label_parent = elem_ctx_container;
        widget_label_para.label_text = GcaleDateStr;
        lv_obj_t *Gcale_fes_sd_label = common_widget_label_create(&widget_label_para);
        lv_obj_align(Gcale_fes_sd_label, LV_ALIGN_TOP_MID, 0, 245);

        if(Hcale_fes_edyf[i])
        {
            memset(GcaleDateStr, 0, sizeof(GcaleDateStr));
            sprintf(GcaleDateStr, "%02dD  %02dM  %d", E_GFes_day, E_GFes_month, E_GFes_year);
            widget_label_para.label_text = GcaleDateStr;
            lv_obj_t *Gcale_fes_ed_label = common_widget_label_create(&widget_label_para);
            lv_obj_align(Gcale_fes_ed_label, LV_ALIGN_TOP_MID, 0, 278);

            widget_img_para.img_x = 70;
            widget_img_para.img_y = 257;
            widget_img_para.img_parent = elem_ctx_container;
            widget_img_para.file_img_dat = hijri_calendar_icon_05_index;
            widget_img_para.img_click_attr = false;
            widget_img_para.event_cb = NULL;
...
static void menu_create_cb(lv_obj_t *obj)
{
    ...
    ui_act_id_t prev_act_id = ui_act_id_Hcalendar_main;
