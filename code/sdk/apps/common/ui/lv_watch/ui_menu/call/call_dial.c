...
static void dial_click_handle(void)
{
    ...
    CtrlCallOutByNum(num_str, num_len);
...
static void lpb_pressed_cb(lv_event_t *e)
{
    ...
    lpb_timer = lv_timer_create(lpb_timer_cb, 150, NULL);
...
static void menu_display_cb(lv_obj_t *obj)
{
    ...
    for(i = 0; i < 10; i++)
    {
        widget_img_para.img_x = button_crd[i].x;
        widget_img_para.img_y = button_crd[i].y;
        widget_img_para.img_parent = obj;
        widget_img_para.file_img_dat = call_08_index;
        widget_img_para.img_click_attr = true;
        widget_img_para.event_cb = click_cb;
        widget_img_para.user_data = (void *)&button_idx[i];
        lv_obj_t *button_container = common_widget_img_create(&widget_img_para, NULL);
        lv_obj_set_ext_click_area(button_container, 5);

        widget_img_para.img_parent = button_container;
        widget_img_para.file_img_dat = comm_num_20x32_wh_00_index + i;
        widget_img_para.img_click_attr = false;
        widget_img_para.event_cb = NULL;
        lv_obj_t *button_num = common_widget_img_create(&widget_img_para, NULL);
    ...
    for(; i < 12; i++)
    {
        ...
        widget_img_para.img_y = button_crd[i].y;
        widget_img_para.img_parent = obj;
        widget_img_para.file_img_dat = spec_button_file[i - 10];
        widget_img_para.img_click_attr = true;
        widget_img_para.event_cb = click_cb;
        widget_img_para.user_data = (void *)&button_idx[i];
        lv_obj_t *button_container = common_widget_img_create(&widget_img_para, NULL);
        ...
        if(i == 10)
        {
            lv_obj_add_event_cb(button_container, lpb_pressed_cb, LV_EVENT_LONG_PRESSED, NULL);
            lv_obj_add_event_cb(button_container, lpb_release_cb, LV_EVENT_RELEASED, NULL);      
        }
    ...
    widget_label_para.user_text_font = &font_common_num_36;
    widget_label_para.label_parent = obj;
    widget_label_para.label_text = num_str;
    num_label = common_widget_label_create(&widget_label_para);
    lv_obj_align(num_label, LV_ALIGN_TOP_MID, 0, 44);
