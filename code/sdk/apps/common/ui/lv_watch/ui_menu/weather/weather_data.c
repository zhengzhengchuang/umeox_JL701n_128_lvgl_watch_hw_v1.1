...
static void no_weather_data_menu_create(lv_obj_t *obj)
{
    ...
    widget_img_para.file_img_dat = weather_no_data_00_index;
    widget_img_para.img_click_attr = false;
    widget_img_para.event_cb = NULL;
    lv_obj_t *no_data_icon = common_widget_img_create(&widget_img_para, NULL);
    lv_obj_align(no_data_icon, LV_ALIGN_TOP_MID, 0, 82);
    ...
    widget_label_para.label_text = get_lang_txt_with_id(lang_txtid_no_data);
    lv_obj_t *no_data_label = common_widget_label_create(&widget_label_para);
    lv_obj_align(no_data_label, LV_ALIGN_TOP_MID, 0, 352);
...
static void has_weather_data_menu_create(lv_obj_t *obj)
{
    ...
    int16_t min0_temper = weather_min_temper(0);

    widget_data_para.data_x = data_sym_x;
    ...
    int16_t max0_temper = weather_max_temper(0);
    ...
    int16_t real_temper = weather_real_temper();
    widget_data_para.data_x = \
        (LCD_WIDTH - data_sym_x) - (4*24 + 12);
    ...
    weather_type_t weather_type = weather_weather_type(0);
    widget_img_para.img_parent = obj;
    widget_img_para.file_img_dat = weather_148x140_00_index + weather_type;
    widget_img_para.img_click_attr = false;
    widget_img_para.event_cb = NULL;
    lv_obj_t *weather_type_icon = common_widget_img_create(&widget_img_para, NULL);
    lv_obj_align(weather_type_icon, LV_ALIGN_TOP_MID, 0, 165);
    ...
    widget_label_para.label_text = \
        get_lang_txt_with_id(lang_txtid_sunny + weather_type);
    lv_obj_t *weather_type_label = common_widget_label_create(&widget_label_para);
    lv_obj_align(weather_type_label, LV_ALIGN_TOP_MID, 0, 332);
...
static void menu_create_cb(lv_obj_t *obj)
{
    ...
    data_valid = Weather_Info.valid;
    ui_mode_t ui_mode = p_ui_info_cache->ui_mode;
    if(ui_mode == ui_mode_watchface)
    {
        ui_act_id_t left_act_id = ui_act_id_hr_sample;
        ui_act_id_t right_act_id = ui_act_id_watchface;
        ui_act_id_t down_act_id;
        if(data_valid)
            down_act_id = ui_act_id_weather_report;
        else
            down_act_id = ui_act_id_null;
...
static void menu_refresh_cb(lv_obj_t *obj)
{
    ...
    bool data_valid_tmp = Weather_Info.valid;
