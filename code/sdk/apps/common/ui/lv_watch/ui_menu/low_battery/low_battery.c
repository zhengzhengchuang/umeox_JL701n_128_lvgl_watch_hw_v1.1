...
static void menu_create_cb(lv_obj_t *obj)
{
    ...
    if(!lang_txt_is_arabic())
        tileview_register_all_menu(obj, ui_act_id_null, ui_act_id_null, \
            prev_act_id, ui_act_id_null, ui_act_id_low_battery);
    else
        tileview_register_all_menu(obj, ui_act_id_null, ui_act_id_null, \
            ui_act_id_null, prev_act_id, ui_act_id_low_battery);
...
static void menu_display_cb(lv_obj_t *obj)
{
    ...
    widget_img_para.file_img_dat = remind_low_bat_00_index;
    widget_img_para.img_click_attr = false;
    widget_img_para.event_cb = NULL;
    widget_img_para.user_data = NULL;
    lv_obj_t *low_bat_icon = common_widget_img_create(&widget_img_para, NULL);
    lv_obj_align(low_bat_icon, LV_ALIGN_TOP_MID, 0, 32);
    ...
    widget_label_para.label_text = get_lang_txt_with_id(lang_txtid_low_battery);
    lv_obj_t *title_label = common_widget_label_create(&widget_label_para);
    lv_obj_align(title_label, LV_ALIGN_TOP_MID, 0, 332);
