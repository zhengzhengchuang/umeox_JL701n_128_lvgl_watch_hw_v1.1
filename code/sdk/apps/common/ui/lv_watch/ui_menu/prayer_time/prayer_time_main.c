...
static void prayer_time_num_container_cb(lv_event_t *e)
{
    if(!e) return;

    ui_menu_jump(ui_act_id_prayer_time_preview);
...
static void prayer_time_reminder_on_cb(lv_event_t *e)
{
    ...
    ui_menu_jump(ui_act_id_prayer_time_remind);

    return;
}

static void prayer_time_reminder_offset_cb(lv_event_t *e)
{
    if(!e) return;

    u8 PT_type = GetPTimeType();
    int32_t val = PT_Cfg.remind_offset[PT_type]/60;
    SetPT_advance(val);

    ui_menu_jump(ui_act_id_prayer_time_advance);
...
static void menu_refresh_cb(lv_obj_t *obj)
{
    ...
    sprintf(PTNumStr, "%02d:%02d", PTResultHour, PTResultMinute);
    common_widget_num_str_refresh(PTTimeNumObj, 5, PTTimeNumDscIdx, \
        PTNumStr, 5, comm_num_46x78_ye_00_index);

    if(am_pm == 0)
        common_widget_img_replace_src(ampm_icon, comm_icon_22_index, ampm_icon_dsc_idx);
    else
        common_widget_img_replace_src(ampm_icon, comm_icon_23_index, ampm_icon_dsc_idx);
    if(time_format == time_format_24)
        lv_obj_add_flag(ampm_icon, LV_OBJ_FLAG_HIDDEN);
    else
        lv_obj_clear_flag(ampm_icon, LV_OBJ_FLAG_HIDDEN);

    uint8_t PTCountdownMode = GetPTimeCountdownMode(PT_type);
    lv_label_set_text(PT_countdown_mode_label, \
        get_lang_txt_with_id(lang_txtid_remaining_time + PTCountdownMode));
    
    u32 PTCountdownTime = GetPTimeCountdownTime(PT_type);
    char PTCountdownTimeStr[10];
    memset(PTCountdownTimeStr, 0, sizeof(PTCountdownTimeStr));
    sprintf(PTCountdownTimeStr, "%02d:%02d:%02d", \
        PTCountdownTime/3600, (PTCountdownTime%3600)/60, (PTCountdownTime%3600)%60);
    common_widget_num_str_refresh(PTCountdownNumObj, 8, \
        PTCountdownNumDscIdx, PTCountdownTimeStr, 8, comm_num_16x28_wh_00_index);
...
static void menu_display_cb(lv_obj_t *obj)
{
    ...
    lv_obj_t *PT_num_container = common_widget_obj_create(&widget_obj_para);
    lv_obj_add_event_cb(PT_num_container, prayer_time_num_container_cb, LV_EVENT_SHORT_CLICKED, NULL);
    ...
    memset(PTNumStr, 0, sizeof(PTNumStr));
    sprintf(PTNumStr, "%02d:%02d", PTResultHour, PTResultMinute);
    num_str_para.parent = PT_num_container;
    num_str_para.num_obj_x = 67;
    num_str_para.num_obj_y = 0;
    num_str_para.p_num_str = PTNumStr;
    num_str_para.num_str_len = 5;
    num_str_para.num_obj = PTTimeNumObj;
    num_str_para.num_obj_max = 5;
    num_str_para.num_dsc_idx = PTTimeNumDscIdx;
    num_str_para.file_00_index = comm_num_46x78_ye_00_index;
    int16_t end_x = common_widget_num_str_create(&num_str_para);

    widget_img_para.img_parent = PT_num_container;
    widget_img_para.img_x = end_x + 10;
    widget_img_para.img_y = 48;
    if(am_pm == 0)
        widget_img_para.file_img_dat = comm_icon_22_index;
    else
        widget_img_para.file_img_dat = comm_icon_23_index;
    widget_img_para.img_click_attr = false;
    widget_img_para.event_cb = NULL;
    ampm_icon = common_widget_img_create(&widget_img_para, &ampm_icon_dsc_idx);
    if(time_format == time_format_24)
        lv_obj_add_flag(ampm_icon, LV_OBJ_FLAG_HIDDEN);
    ...
    widget_label_para.long_mode = LV_LABEL_LONG_SCROLL;
    widget_label_para.text_align = LV_TEXT_ALIGN_CENTER;
    widget_label_para.label_text_color = lv_color_hex(0x999999);
    widget_label_para.label_ver_center = false;
    widget_label_para.user_text_font = NULL;
    widget_label_para.label_parent = obj;
    widget_label_para.label_text = \
        get_lang_txt_with_id(lang_txtid_remaining_time + PTCountdownMode);
    PT_countdown_mode_label = common_widget_label_create(&widget_label_para);
    ...
    num_str_para.num_obj_x = 128;
    num_str_para.num_obj_y = 314;
    num_str_para.p_num_str = PTCountdownTimeStr;
    num_str_para.num_str_len = 8;
    num_str_para.num_obj = PTCountdownNumObj;
    num_str_para.num_obj_max = 8;
    num_str_para.num_dsc_idx = PTCountdownNumDscIdx;
    num_str_para.file_00_index = comm_num_16x28_wh_00_index;
    ...
    widget_img_para.file_img_dat = prayer_time_01_index;
    widget_img_para.img_click_attr = true;
    widget_img_para.event_cb = prayer_time_reminder_offset_cb;
    widget_img_para.user_data = NULL;
    lv_obj_t *reminder_offset_icon = common_widget_img_create(&widget_img_para, NULL);
    ...
    if(menu_align == menu_align_left)
        ...
        widget_img_para.img_x = 44;   
    if(remind_en)
        widget_img_para.file_img_dat = prayer_time_03_index;
    else
        widget_img_para.file_img_dat = prayer_time_02_index;
    widget_img_para.event_cb = prayer_time_reminder_on_cb;
    lv_obj_t *reminder_on_icon = common_widget_img_create(&widget_img_para, NULL);
