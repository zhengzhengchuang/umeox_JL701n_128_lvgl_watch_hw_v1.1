...
static int smartbox_extra_flash_opt_insert_start(u8 *file_name, u16 file_name_len, u32 file_size)
{
    ...
    // 防止外挂flash存放非表盘文件时，因为长文件名触发下面的断言
    if (0 != strncmp(file_name + 1, "bgp_w", strlen("bgp_w")) &&
        0 != strncmp(file_name + 1, "BGP_W", strlen("BGP_W")) &&
        0 != strncmp(file_name + 1, "watch", strlen("watch")) &&
        0 != strncmp(file_name + 1, "WATCH", strlen("WATCH"))) {
        return 0;
    }
...
*/
/*----------------------------------------------------------------------------*/
static u8 g_ex_flash_file_flag = 0;
void smartbox_ex_flash_file_flag(u8 flag)
{
    g_ex_flash_file_flag = flag;
}
u8 smartbox_eflash_flag_get(void)
{
    ...
    if (0xFF == eflash_flag) {
#if RCSP_UPDATE_EN
        if (0 == get_update_ex_flash_flag() && 0 == get_jl_update_flag() && 0 == g_ex_flash_file_flag)
#endif
        {
            smartbox_eflash_flag_set(0);
            return 0;
...
static int smartbox_extra_flash_ota_update_state(u8 *data, u16 *data_len, u8 offset, u8 state)
{
    ...
    if (smart == NULL) {
        result = -1;
    } else if (smart->low_battery_level && (get_self_battery_level() > (smart->low_battery_level % 10))) { // 防止low_battery_level设置大于10
...
void smartbox_update_flag_clear(void)
{
#if RCSP_UPDATE_EN
