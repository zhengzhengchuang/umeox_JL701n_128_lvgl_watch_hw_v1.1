...
static u32 common_function_attr_dev_info_get(void *priv, u8 attr, u8 *buf, u16 buf_size, u32 offset)
{
    ...
    if (smart->file_transfer_mode && dev_manager_get_root_path_by_logo("virfat_flash")) {
        printf("dev [virfat_flash] online\n");
        dev_info.status |= BIT(BS_FLASH);
        dev_info.flash_handle = app_htonl((u32)BS_FLASH);
    }

    if (smart->file_trans_nor_fat && dev_manager_get_root_path_by_logo("fat_nor")) {
        printf("dev [fat_nor] online\n");
        dev_info.status |= BIT(BS_FLASH_2);
        dev_info.flash_handle_2 = app_htonl((u32)BS_FLASH_2);
    }
...
static bool smartbox_common_function_set(void *priv, u8 *data, u16 len)
{
    ...
    if (smart->set_err_code) {
        smart->set_err_code = 0;
        return false;
    }
    return true;
}
...
void smartbox_update_dev_state(u32 event, void *param)
{
#if JL_SMART_BOX_EXTRA_FLASH_OPT
    extern bool smartbox_exflash_flash_opt_check(void *param);
    if (smartbox_exflash_flash_opt_check(param)) {
        return;
    }
#endif
    switch (event) {
