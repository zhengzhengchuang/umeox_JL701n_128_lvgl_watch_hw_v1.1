...
typedef enum __DEVICE_REFRESH_FW_STATUS {
    ...
    DEVICE_UPDATE_STA_EX_FLASH_ERR = 0xB,   //外挂flash升级失败
...
static u8 update_result_handle(u8 err)
{
    ...
    if (err & UPDATE_RESULT_FLAG_BITMAP) {
        ...
        case UPDATE_RESULT_FLASH_DATA_VERIFY_ERR:
            ...
            break;
        case DEVICE_UPDATE_STA_EX_FLASH_ERR:
            res = DEVICE_UPDATE_STA_EX_FLASH_ERR;
            break;
        default:
            res = err;
            break;
...
void rcsp_update_ancs_disconn_handler(void)
{
    ...
    if (support_user_file_update_v2_en) {
        extern void user_file_flash_update_len_clear(void);
        extern void user_file_exit_write_task(void);
        user_file_flash_update_len_clear();
        user_file_exit_write_task();
    } else
#endif
    {
        extern void ex_flash_update_len_clear(void);
        extern void ex_flash_exit_write_task(void);
        ex_flash_update_len_clear();
        ex_flash_exit_write_task();
    }
...
static void rcsp_update_state_cbk(int type, u32 state, void *priv)
{
    ...
    case UPDATE_CH_EXIT:
        ...
        // 升级结束后，还原rcsp等待命令响应的超时时间
        JL_protocol_timeout_resume();
        break;
    case EX_UPDATE_CH_READ_TO:
        printf("EX_UPDATE_CH_READ_TO");
        int read_timeout = *((int *)priv);
        // 设置外挂flash升级的超时时间
#if (USER_FILE_UPDATE_V2_EN)
        if (support_user_file_update_v2_en) {
            extern void user_file_flash_read_timeout_set(int read_timeout);
            user_file_flash_read_timeout_set(read_timeout);
        } else
#endif
        {
            extern void ex_flash_read_timeout_set(int read_timeout);
            ex_flash_read_timeout_set(read_timeout);
        }
        // 设置rcsp等待命令响应的超时时间；这里的read_timeout值以10ms为一个单位的超时时间，
        // rcsp超时时间1ms为一个单位，所以需要*10
        JL_protocol_timeout_set(read_timeout * 10);
        // 设置update里面升级的超时时间，*((int *)priv) = 0开启时间的设置
        *((int *)priv) = 0;
        break;
    }
}
