...
#define EMITTER_CONNECT_TIMEOUT_MS			(60*1000)
...
static u8 volatile emitter_dac_mute_en = 0;
...
void bt_emitter_audio_set_mute(u8 mute)
{
    ...
    g_printf("=======mute:%d %d==============", mute, emitter_dac_mute_en);
}

u8 bt_emitter_audio_get_mute()
{
    return emitter_dac_mute_en;
}
...
extern void bredr_search_device();
extern void bredr_conn_dev(u8 *mac);
void emitter_bt_connect(u8 *mac)
{
    ...
    while (hci_standard_link_check() != 0) {
        //wait disconnect;
        os_time_dly(10);
    }
#endif
...
u8 connect_last_sink_device_from_vm()
{
    ...
    if (flag) {
        //connect last conn
        printf("last sink device addr from vm:");
        put_buf(mac_addr, 6);
...
#endif
        ////开启搜索设备
        ////为了省功耗，平时不开启bredr不回链接和开搜索，通过ble 来开启bredr
...
void bt_emitter_rx_bulk_change(u8 mode)
{
#if 0
    extern void link_conn_follow_ctrl(u8 * addr, u8 follow);
    extern void radio_set_eninv(int v);

    bit_clr_ie(IRQ_BREDR_IDX);
    bit_clr_ie(IRQ_BT_CLKN_IDX);
    /* radio_set_eninv(0); */
    link_conn_follow_ctrl(NULL, 1);
    bredr_bulk_change(mode);
    link_conn_follow_ctrl(NULL, 0);
    bit_set_ie(IRQ_BREDR_IDX);
    bit_set_ie(IRQ_BT_CLKN_IDX);
    /* radio_set_eninv(1); */
...
#if TCFG_USER_BLE_CTRL_BREDR_EN
u32 emitter_connect_timer_id;
void emitter_connect_timeout(void *pive)
{
...
void emitter_search_stop(u8 result)
{
    ...
    if (wait_connect_flag) {
        ...
        if (get_total_connect_dev() == 2) {
            user_send_cmd_prepare(USER_CTRL_WRITE_SCAN_DISABLE, 0, NULL);
            user_send_cmd_prepare(USER_CTRL_WRITE_CONN_DISABLE, 0, NULL);
        } else if (get_total_connect_dev() == 1 && get_bt_connect_status() != BT_STATUS_WAITINT_CONN) {
            user_send_cmd_prepare(USER_CTRL_WRITE_SCAN_DISABLE, 0, NULL);
            user_send_cmd_prepare(USER_CTRL_WRITE_CONN_ENABLE, 0, NULL);
        } else {
            user_send_cmd_prepare(USER_CTRL_WRITE_SCAN_ENABLE, 0, NULL);
...
#if TCFG_USER_BLE_CTRL_BREDR_EN
        if (emitter_connect_timer_id) {
            sys_timeout_del(emitter_connect_timer_id);
            emitter_connect_timer_id = 0;
        }
        emitter_connect_timer_id = sys_timeout_add(NULL, emitter_connect_timeout, EMITTER_CONNECT_TIMEOUT_MS);
#endif
#else
        user_send_cmd_prepare(USER_CTRL_WRITE_SCAN_DISABLE, 0, NULL);
        if (!result) {
...
u8 bt_emitter_stu_set(u8 *addr, u8 on)
{
    ...
    bt_emitter_audio_set_mute(on);
    while ((on == 0) && bt_emitter_stu_get()) {
        os_time_dly(2);
        putchar('H');
    }
    return on;
...
u8 bt_emitter_pp(u8 pp)
{
    ...
    if (pp && bt_get_music_device_style() != SET_MUSIC_IN_WATCH) {
        printf("current_dev_is_not_emiter \n");
        return 0;
    }
...
void emitter_save_remote_name(u8 *addr, u8 *name)
{
    ...
    while (1) {
        ...
        id++;
        if (id > (CFG_REMOTE_DN_00 + 19)) {
            id = (CFG_REMOTE_DN_00 + 19);
            break;
        }
    }
...
*/
/*----------------------------------------------------------------------------*/
void emitter_search_remote(void *cb_priv, int (*cb)(void *priv, u8 *mac, u8 *name, u8 *out_del, u8 *out_exit))
{
    struct remote_name remote_n;
    u16 id = CFG_REMOTE_DN_00;
    u8 del, exit;
    while (1) {
        ...
        if (remote_n.crc == CRC16((u8 *)&remote_n.addr, sizeof(struct remote_name) - 2)) {
            del = exit = 0;
            cb(cb_priv, remote_n.addr, remote_n.name, &del, &exit);
            if (del) {
...
*/
/*----------------------------------------------------------------------------*/
void emitter_delete_remote_all(void)
{
    struct remote_name remote_n;
    u16 id = CFG_REMOTE_DN_00;
    while (1) {
