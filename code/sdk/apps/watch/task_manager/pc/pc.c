...
static int usb_pc_offline_timer = 0;
...
static char usb_mode_type[8];
void usb_pc_offline_det(void *priv)
{
    static u8 host_cnt = 0;
    static u8 slave_cnt = 0;
    static u16 last_sof_frame = 0;
    u16 sof_frame = usb_read_sofframe(USB0);
    u32 dm_state = usb_read_dm_se(USB0);

    gpio_set_pull_up(IO_PORT_DM + 2 * USB0, 1);
    if (usb_read_dm_se(USB0)) {

        if (dm_state && (sof_frame == last_sof_frame)) {
            host_cnt = 0;
            slave_cnt ++;
        } else {
            last_sof_frame = sof_frame;
            slave_cnt = 0;
        }
    } else {
        slave_cnt = 0;
    ...
    if (slave_cnt > TCFG_OTG_SLAVE_OFFLINE_CNT) {
        g_printf("usb pc offline!!!");
        struct sys_event event;
        event.type = SYS_DEVICE_EVENT;
        event.arg = (void *)DEVICE_EVENT_FROM_OTG;
        event.u.dev.event = DEVICE_EVENT_OUT;
        sprintf(usb_mode_type, "s:%d", USB0);
        event.u.dev.value = (int)(usb_mode_type);
...
#endif
#if (TCFG_OTG_MODE == 0)
    if (usb_pc_offline_timer == 0) {
        usb_pc_offline_timer = sys_timer_add(NULL, usb_pc_offline_det, 100);
    }
#endif
}
...
{
#if (TCFG_OTG_MODE == 0)
    if (usb_pc_offline_timer) {
        sys_timer_del(usb_pc_offline_timer);
        usb_pc_offline_timer = 0;
    }
#endif
    pc_app_uninit();
...
int pc_app_check(void)
{
    ...
    u32 jiffs = 0;
    u8 status = true;
    usb_hotplug_enable(0, OTG_SLAVE_MODE);
#ifdef CONFIG_CPU_BR28
    //DM上拉电阻180k，接PC会读到低电平
    gpio_set_pull_up(IO_PORT_DM, 1);
    gpio_set_pull_down(IO_PORT_DM, 0);
    os_time_dly(1); //延时一下让电平稳定（需>22us）
    for (jiffs = 0; jiffs < 5; jiffs++) 
    {
    ...
    if (status == true) 
    {
        ...
        os_time_dly(2);
        if (gpio_read(IO_PORT_DP)) {
            //判断为充电器，但是有些充电器会误判，没有定时器检查pc的sof那么准确
            status = false;
        }
        gpio_set_direction(IO_PORT_DM, 1);
    }
    usb_hotplug_disable(0);
    //把DP&DM设为给高阻，减小功耗
    ...
    log_info("pc_app_check %d", status);
