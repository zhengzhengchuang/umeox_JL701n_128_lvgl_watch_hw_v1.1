...
void cfg_file_parse(u8 idx)
{
    ...
    log_info_hexdump((u8 *)&bt_cfg.tws_device_indicate, 2);
...
#if TCFG_APP_BT_EN
    /* bt_max_pwr_set(app_var.rf_power, 5, 8, 10); // 695N:0-4..701N:0-8 */
    bt_max_pwr_set(app_var.rf_power, 5, 7, 7); // 695N:0-4..701N:0-8
#endif
    ...
    if (ret > 0) {
        log_info_hexdump((u8 *)&aec, sizeof(AEC_CONFIG));
    ...
    if (ret > 0) {
        ...
        log_info_hexdump((u8 *)&status_config, sizeof(STATUS_CONFIG));
    }
#endif
