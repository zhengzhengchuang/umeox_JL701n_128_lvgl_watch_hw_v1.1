...
const int CONFIG_VISUAL_BT_RF_POWER_LEVEL    = 0;
...
const int config_btctler_le_roles    = (LE_SCAN | LE_INIT | LE_MASTER | LE_ADV | LE_SLAVE);//watch添加主机行为
#else
...
#else
#if SMBOX_MULTI_BLE_EN
const int config_btctler_le_hw_nums = CONFIG_BT_GATT_CONNECTION_NUM;//watch设置主从机个数
#else
...
// Master AFH
#if SMBOX_MULTI_BLE_EN
const int config_btctler_le_afh_en = 1;//watch 添加了主机行为
// LE RAM Control
const int config_btctler_le_rx_nums = 10;
const int config_btctler_le_acl_packet_length = 251;
const int config_btctler_le_acl_total_nums = 10;//watch 增加了主机行为,将acl从原来的5设置为25
#else
const int config_btctler_le_afh_en = 0;//watch 添加了主机行为
// LE RAM Control
const int config_btctler_le_rx_nums = 8;
const int config_btctler_le_acl_packet_length = 251;
...
#if (defined(CONFIG_BT_RX_BUFF_SIZE) && (CONFIG_BT_RX_BUFF_SIZE < (10*1024)))
const int CONFIG_A2DP_DELAY_TIME            = (CONFIG_BT_RX_BUFF_SIZE - 1200) * 1000 / 44100;
#else /* #if (defined(CONFIG_BT_RX_BUFF_SIZE) && (CONFIG_BT_RX_BUFF_SIZE < (10*1024))) */
const int CONFIG_A2DP_DELAY_TIME            = 340;
#endif /* #if (defined(CONFIG_BT_RX_BUFF_SIZE) && (CONFIG_BT_RX_BUFF_SIZE < (10*1024))) */
...
// Master multi-link
#if SMBOX_MULTI_BLE_EN
const int config_btctler_le_master_multilink = 1;//watch 增加为一主一从属于多机行为
#else
